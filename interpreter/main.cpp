...
int main(int argc, char** argv) {
    ...
    assert(argc == 5, "USAGE: ./mipsinterpret <MIPS src> <compile output> <memory output> <lab3_test.py location>");
    ...
    if (!pid) {
        cout << "Forking into python3 " << argv[4] << "..." << endl;
        const char* vp_argv[] = {"python3", argv[4], argv[2], argv[3], NULL};
        execvp(vp_argv[0], (char* const*)vp_argv);
        perror("execvp");
        error("Bad execvp call.");
    }
